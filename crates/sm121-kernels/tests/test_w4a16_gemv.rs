...
fn w4a16_nvfp4_gemv() {
    ...
    // Smaller test: K=512, N=2048 (multiple of 16)
    let k: u32 = 512;
    let n: u32 = 2048;
    ...
    // Quantize B to NVFP4 using existing kernel: per-row, blocks of 16 along N.
    // Layout: out [K, N/2] u8 ; scales [K, N/16] u8
    let mut b_packed = stream.alloc_zeros::<u8>((k * n / 2) as usize).unwrap();
    let mut b_scales = stream.alloc_zeros::<u8>((k * n / 16) as usize).unwrap();
    quantization::quant_bf16_to_nvfp4(&ctx, &stream, &b_dev, &mut b_packed, &mut b_scales, k, n)
        .expect("quantize NVFP4");

    let mut out_f32 = stream.alloc_zeros::<f32>(n as usize).unwrap();
    let num_shards = 2;
    gemm::gemv_w4a16_nvfp4_split_k(
        &ctx,
        &stream,
        &x_dev,
        &b_packed,
    ...
    // FP4 has 4 levels of mantissa (3 bits incl sign). Per-block scaling helps,
    // but expect significantly larger error than W8A16 (which was 0.18% mean).
    assert!(rel_max < 0.40, "relative max diff too large: {rel_max}");
    assert!(rel_mean < 0.10, "relative mean diff too large: {rel_mean}");
}
