//! Scaling benchmark for FP8 GEMM v1 vs v3.
...
fn main() {
    ...
    println!(
        "{:<18} {:>9} {:>9} {:>9} {:>9} {:>9} {:>9} {:>7}",
        "shape", "v1(us)", "v3(us)", "v3.5(us)", "v1 TF", "v3 TF", "v3.5 TF", "v35/v3"
    );
    println!("{}", "-".repeat(85));

    for &(m, n, k) in configs {
        let a = stream.alloc_zeros::<u8>((m * k) as usize).unwrap();
        let b = stream.alloc_zeros::<u8>((k * n) as usize).unwrap();
        let mut c = stream.alloc_zeros::<u16>((m * n) as usize).unwrap();

        let v3_ok = m.is_multiple_of(128) && n.is_multiple_of(128);
        let v35_ok = v3_ok && k >= 96;
        ...
        let tf_v35 = flops / (ms_v35 as f64 * 1e-3) / 1e12;
        let speedup = ms_v3 / ms_v35;
